...
@variables x
@variables c0, c1, c2, c3, c4, c5, c6
const PREC_BITS = 256
const CC = AcbField(PREC_BITS)

F = [c6*x^6+c5*x^5+c4*x^4+c3*x^3+c2*x^2+c1*x+c0]
bp = [CC(1/2), CC(9/7), CC(9/7), CC(45/7), CC(45/56), CC(-2/3), CC(1/3)]
x0 = [CC(.0720838,-.481488)]
v1 = vertex(bp,[x0])

vars = [x]
pars = [c0, c1, c2, c3, c4, c5, c6]
v1 = vertex(bp,[x0])
deg = length(pars)-1
...
for n_nodes in 3:6
    result_name = "results_degree_6_$(n_nodes)_nodes_new.txt"
